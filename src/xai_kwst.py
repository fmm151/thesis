...
def train_model(X_train, y_train, algorithm):
    ...
    if algorithm == "xgboost":
        ...
        history = model.fit(X_train, y_train.values.ravel(), validation_split=0.2, epochs=100, batch_size=512,
...
if __name__ == "__main__":
    ...
    explain_with_shap_dependence_plots(model_gs[algorithm], model_shap_values, family, test_sample[family],
                                       "Reputation", "Length", "Words_Mean", "Max_Let_Seq", "Words_Freq", "Vowel_Freq",
                                       "Entropy", "DeciDig_Freq", "Max_DeciDig_Seq", algorithm)
    explain_with_force_plots(model_gs[algorithm], model_shap_values, family, test_sample[family], names_sample[family],
                             algorithm, model_explainer[algorithm])
    ...
    for algorithm in algorithms:
        for family in per_category_test.keys():
            if (algorithm == "xgboost" and (family == "all_DGAs" or family == "tranco")) or (
                    algorithm == "mlp" and family in paper_families):
                continue
