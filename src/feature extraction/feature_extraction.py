...
def load_tranco_names():
    filename = "../../files/tranco_filtered_files/tranco_top100k.txt"
    names = set()
    fdr = open(filename, "r")
    sequence = 0
    for line in fdr:
...
def create_csv_line(f1, f2, f3, f4_group, f5, f6, f7, f8, f9, f10, f11, f12, f13, f14, f15, name_original, label,
                    family):
    csv_line = ""
    csv_line += str(f1) + ","
    csv_line += str(f2) + ","
...
def export_features(name, name_original, label, family, whitelist):
    ...
    csv_line = create_csv_line(length, max_digit_sequence, max_string_sequence, individual_string_char_frequency,
                               special_char_freq, ratio_special_char, integer_freq, ratio_integers, vowels_freq,
                               vowels_ratio, max_gap, reputation, words_number, words_mean, shannon_entropy,
                               name_original, label, family)
    return csv_line
